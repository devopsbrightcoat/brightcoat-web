...
// Se usa tanto desde Gastos como desde Planillas — ambas comparten la misma
// plantilla (/public/plantilla-gastos.xlsx) e importador; la diferencia es
// solo cómo se filtra la tabla después (category === 'labor' o no).
      ...
      <div className="space-y-4">
        ...
        {stage === 'idle' && (
          <ImportDropzone
            ...
            acceptHint='Plantilla "Gastos" en formato .xlsx (materiales, mano de obra, transporte, herramientas y otros)'
          />
        )}
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row.rowNumber} className="border-b border-white/5 last:border-0">
                        ...
                        <td className="px-4 py-2.5 text-ink-200">{row.propertyName || 'Gasto general'}</td>
                        <td className="px-4 py-2.5 text-ink-400">{categoryLabels[row.category] ?? row.category}</td>
