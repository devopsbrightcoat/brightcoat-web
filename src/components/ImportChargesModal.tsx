...
type ImportChargesModalProps = {
  open: boolean
  onClose: () => void
  onImported: () => void
}

export const ImportChargesModal = ({ open, onClose, onImported }: ImportChargesModalProps) => {
  const [stage, setStage] = useState<Stage>('idle')
  const [fileName, setFileName] = useState('')
  const [parseError, setParseError] = useState<string | null>(null)
  const [rows, setRows] = useState<ValidatedChargeRow[]>([])
  const [progress, setProgress] = useState({ done: 0, total: 0 })
  const [outcomes, setOutcomes] = useState<ImportChargeOutcome[]>([])
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (parsed.length === 0) {
        setParseError('No se encontraron filas con datos en la hoja "Cobros".')
        setRows([])
        setStage('idle')
        return
      }
      setRows(parsed.map(validateChargeRow))
      setStage('parsed')
    } catch (err) {
  ...
  const handleImport = async () => {
    ...
    const result = await importValidatedChargeRows(validRows, (done, total) => setProgress({ done, total }))
    setOutcomes(result)
    setStage('done')
    onImported()
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Cargar Excel de cobros" widthClassName="max-w-3xl">
      <div className="space-y-4">
        <a
          href="/plantilla-cobros.xlsx"
          download
        ...
        {stage === 'idle' && (
          <ImportDropzone onFile={handleFile} parseError={parseError} acceptHint='Plantilla "Cobros" en formato .xlsx' />
        )}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5 text-xs uppercase tracking-wide text-ink-500">
                    <th className="px-4 py-2.5 font-medium">Pestaña</th>
                    <th className="px-4 py-2.5 font-medium">Tabla</th>
                    <th className="px-4 py-2.5 font-medium">Apartamento</th>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={`${row.sheetName}-${row.rowNumber}-${row.side}`} className="border-b border-white/5 last:border-0">
                        ...
                        <td className="px-4 py-2.5 text-ink-400">{row.status === 'paid' ? 'Subido a OPS' : 'Pendiente'}</td>
                        <td className="px-4 py-2.5 text-ink-200">{row.unitLabel || '—'}</td>
