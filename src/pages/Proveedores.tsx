...
import { AddVendorModal } from '../components/proveedores/AddVendorModal'
import { EditVendorModal } from '../components/proveedores/EditVendorModal'
...
export const Proveedores = () => {
  ...
  const { data: vendors, loading, error } = useSupabaseQuery(fetchVendors, [refreshKey])
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setEditingVendor(info.row.original)}
            ...
            <button
              type="button"
              onClick={() => setDeletingVendor(info.row.original)}
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      <PageHeader
        title="Proveedores"
        subtitle="Catálogo de proveedores — de dónde sale cada compra registrada en Gastos"
        action={
          <button
            type="button"
      ...
      <div className="mx-8 mt-6 mb-6 flex flex-1 min-h-0 max-w-3xl flex-col overflow-hidden rounded-xl border border-white/10 bg-surface-alt">
        {loading ? (
          ...
          <p className="px-5 py-6 text-sm text-ink-500">Todavía no hay proveedores.</p>
        ) : (
          <>
      ...
      <AddVendorModal open={addOpen} onClose={() => setAddOpen(false)} onSaved={() => setRefreshKey((k) => k + 1)} />
