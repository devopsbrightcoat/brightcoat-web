...
const columnHelper = createColumnHelper<ChargeTemplate>()
...
export const ConfiguracionCobrosFijos = () => {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor((row) => propertyNameById.get(row.propertyId) ?? '—', {
        id: 'property',
        header: 'Propiedad',
      }),
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      <PageHeader
        title="Cobros fijos"
        subtitle="Catálogo de cargos recurrentes para usar como plantilla al agregar un cobro fijo"
        action={
          <button
            type="button"
      ...
      <div className="mx-8 mt-6 mb-6 flex flex-1 min-h-0 max-w-3xl flex-col overflow-hidden rounded-xl border border-white/10 bg-surface-alt">
        {loading ? (
          ...
          <p className="px-5 py-6 text-sm text-ink-500">Todavía no hay cobros fijos.</p>
        ) : (
          <>
      ...
      <ConfirmModal
        ...
        onConfirm={async () => {
          ...
          await deleteChargeTemplate(deletingTemplate.id)
          setRefreshKey((k) => k + 1)
        }}
      />
    </div>
