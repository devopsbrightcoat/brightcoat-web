...
export const Planillas = () => {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor(
        (row) => (row.employeeId ? employees?.find((e) => e.id === row.employeeId)?.name : undefined) ?? '—',
        { id: 'employee', header: 'Empleado' },
      ),
      columnHelper.accessor(
        (row) => (row.propertyId ? properties?.find((p) => p.id === row.propertyId)?.name : 'General') ?? '—',
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      <PageHeader
        title="Planillas"
        subtitle="Pago de mano de obra por propiedad"
        action={
          <button
            type="button"
